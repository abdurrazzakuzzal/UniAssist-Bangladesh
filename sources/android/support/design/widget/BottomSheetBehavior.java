...
public class BottomSheetBehavior<V extends View> extends Behavior<V> {
    ...
    public BottomSheetBehavior(Context context, AttributeSet attrs) {
        super(context, attrs);
        TypedArray a = context.obtainStyledAttributes(attrs, C0020R.styleable.BottomSheetBehavior_Layout);
        TypedValue value = a.peekValue(C0020R.styleable.BottomSheetBehavior_Layout_behavior_peekHeight);
        if (value == null || value.data != -1) {
            setPeekHeight(a.getDimensionPixelSize(C0020R.styleable.BottomSheetBehavior_Layout_behavior_peekHeight, -1));
        } else {
            setPeekHeight(value.data);
        }
        setHideable(a.getBoolean(C0020R.styleable.BottomSheetBehavior_Layout_behavior_hideable, false));
        setSkipCollapsed(a.getBoolean(C0020R.styleable.BottomSheetBehavior_Layout_behavior_skipCollapsed, false));
        a.recycle();
    ...
    public boolean onLayoutChild(CoordinatorLayout parent, V child, int layoutDirection) {
        ...
        if (this.mPeekHeightAuto) {
            if (this.mPeekHeightMin == 0) {
                this.mPeekHeightMin = parent.getResources().getDimensionPixelSize(C0020R.dimen.design_bottom_sheet_peek_height_min);
            }
    ...
    public void onStopNestedScroll(CoordinatorLayout coordinatorLayout, V child, View target) {
        ...
        if (child.getTop() == this.mMinOffset) {
            ...
            if (this.mLastNestedScrollDy > 0) {
                top = this.mMinOffset;
                targetState = 3;
            } else if (this.mHideable != 0 && shouldHide(child, getYVelocity())) {
